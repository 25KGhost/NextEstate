...
export const fmt = {
  ...
  datetime: (s) => new Date(s).toLocaleString('en-US', {
...
export function propertyCard(p, opts = {}) {
  ...
  const slug = p.slug;
  // status-active is intentionally not highlighted — only non-active get a badge
  const statusBadgeClass = { pending: 'status-pending', sold: 'status-sold' };

  return `
    <article class="prop-card ${opts.featured ? 'featured' : ''}" data-slug="${slug}">
      <a href="/properties/${slug}" class="card-link">
        <div class="card-img">
          <img src="${img}" alt="${escHtml(p.title || p.address)}" loading="lazy">
          <span class="badge intent-${p.intent.toLowerCase()}">${p.intent}</span>
          ${p.status !== 'active' ? `<span class="badge status-badge ${statusBadgeClass[p.status] || ''}">${p.status}</span>` : ''}
        ...
        <div class="card-footer">
          <span class="card-agency">${escHtml(p.agency || 'Lucent Estate')}</span>
...
export function inquiryFormHTML(propertySlug = '', propertyTitle = '') {
  return `
    <div class="modal-header">
      <h3>Contact Agent</h3>
      <p>${propertyTitle ? `Enquiring about: <strong>${escHtml(propertyTitle)}</strong>` : 'Send an inquiry or schedule a tour'}</p>
    </div>
    <form id="inquiry-form" onsubmit="handleInquirySubmit(event)">
      <input type="hidden" name="property_slug" value="${escHtml(propertySlug)}">
      <div class="form-row">
...
// ── Favourites (localStorage) — FIXED: toggle returns correct boolean ──
export const favs = {
  get: () => {
    try { return JSON.parse(localStorage.getItem('lucent_favs') || '[]'); }
    catch { return []; }
  },
  toggle: (id) => {
    let f = favs.get();
    const had = f.includes(id);
    f = had ? f.filter(x => x !== id) : [...f, id];
    try { localStorage.setItem('lucent_favs', JSON.stringify(f)); } catch {}
    return !had; // true = now added
  },
...
export function syncFavButtons() {
  document.querySelectorAll('.fav-btn').forEach(btn => {
    btn.classList.toggle('active', favs.has(btn.dataset.id));
  });
}
