...
export const api = {
  ...
  admin: {
    ...
    getInquiries: (filters = {}) => apiFetch('get-inquiries', filters),
...
export const auth = {
  ...
  login: (password) => {
    // Admin password check — the serverless function validates the actual token
    // We store what user typed; first request that fails means wrong password
    sessionStorage.setItem('lucent_admin_token', password);
  },
  logout: () => {
    sessionStorage.removeItem('lucent_admin_token');
